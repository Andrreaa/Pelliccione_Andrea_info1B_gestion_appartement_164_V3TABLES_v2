...
##### Cette démo. CRUD (Create Read Update Delete) presque complète sur les 3 tables de la base adresse : Soit "t_adresse"; "t_personne" et la table intermédiaire "t_genre_film"
...
## Situation 2 : Vous avez déjà commencé à remplacer mes requêtes par les vôtres depuis le 14.04 2022
  ...
  * Dans "PyCharm", cliquer avec le bouton Nom_personne de la souris sur le nom de la racine de votre projet et choisir "Compare With..." (CTRL-D", vous devez importer tout ce qui a changé dans la nouvelle version que je vous propose, mais ne pas écraser ce que vous avez déjà fait.
...
* Dans "PyCharm", importer la BD à partir du fichier DUMP
    ...
    * Cliquer avec le bouton Nom_personne sur l'onglet de ce fichier et choisir "run" (CTRL-MAJ-F10)
...
A) Modifier MES requêtes AFFICHER (READ) du CRUD(Create Read Update Delete) sur la table "t_personne"
  *	Ouvrir le fichier "genres/gestion_genres_crud.py"
  * Modifier les requêtes "SELECT" sur la table "t_personne" par des "SELECT" sur une de vos "t_????"
  * Modifier dans le répertoire "TEMPLATES" la page html consacrée à l'affichage pour ma "t_personne", il faut ouvrir "templates/genres_afficher.html"
...
B) Une fois que l'affichage est fonctionnel et uniquement dans ce cas :
  * Vous devez alors changer les noms de variables en Python pour les adapter à votre sujet
    * Exemple : "strsql_droit_afficher" doit s'écrire avec un nom qui correspond mieux à votre sujet de BD

C) Faites souvent des "COMMITS" et des "PUSH" sur votre "GITHUB" (VOTRENOM_VOTREPRENOM_VOTRECLASSE_VOTRESUJET_164_V3TABLES)

D) Recommencer les étapes A) à C) pour l'action "CREATE" pour la table "t_personne"

E) Recommencer les étapes A) à C) pour l'action "UPDATE" pour la table "t_personne"

F) Recommencer les étapes A) à C) pour l'action "DELETE" pour la table "t_personne"
