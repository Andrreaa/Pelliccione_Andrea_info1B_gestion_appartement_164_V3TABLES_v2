...
@app.route("/avoir_droit_afficher/<int:id_details_sel>", methods=['GET', 'POST'])
def avoir_droit_afficher(id_details_sel):
    print(" avoir_droit_afficher id_details_sel ", id_details_sel)
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT Id_personne, Nom_personne, Prenom_personne, Date_naissance_personne,
                ...
                if id_details_sel == 0:
                    ...
                    valeur_id_film_selected_dictionnaire = {"value_id_film_selected": id_details_sel}
                ...
                if not data_genres_films_afficher and id_details_sel == 0:
                    flash("""La table "t_film" est vide. !""", "warning")
                elif not data_genres_films_afficher and id_details_sel > 0:
    ...
    print("avoir_droit_afficher  ", data_genres_films_afficher)
    # Envoie la page "HTML" au serveur.
    return render_template("avoir_droit/avoir_droit_afficher.html", data=data_genres_films_afficher)
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT id_droit, droit FROM t_droit ORDER BY id_droit ASC"""
            ...
            id_film_genres_edit = request.values['id_avoir_personne_edit_html']
            ...
            lst_data_genres_films_non_attribues = [item['droit'] for item in data_genres_films_non_attribues]
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            strsql_insert_genre_film = """INSERT INTO t_avoir_droit (id_avoir_droit, Fk_droit, Fk_personne)
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """
                                   SELECT Id_personne, nom_personne, Prenom_personne, Date_naissance_personne, GROUP_CONCAT(id_droit) as AvoirDroit FROM t_avoir_droit
        ...
        strsql_genres_films_non_attribues = """
                                                SELECT id_droit, droit FROM t_droit WHERE id_droit not in(SELECT Id_personne as idAvoirDroit FROM t_avoir_droit
                                                    INNER JOIN t_personne ON t_personne.Id_personne = t_avoir_droit.Fk_personne
                                                    INNER JOIN t_droit ON t_droit.id_droit = t_avoir_droit.fk_droit
                                                    WHERE Id_personne = %(value_id_film_selected)s)
                                            """

        strsql_genres_films_attribues = """SELECT Id_personne, id_droit, droit FROM t_avoir_droit
                                            INNER JOIN t_personne ON t_personne.Id_personne = t_avoir_droit.Fk_personne
                                            INNER JOIN t_droit ON t_droit.id_droit = t_avoir_droit.Fk_droit
                                            WHERE Id_personne = %(value_id_film_selected)s"""
