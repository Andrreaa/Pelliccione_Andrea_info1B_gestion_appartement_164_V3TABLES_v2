...
def pieces_afficher(order_by, id_genre_sel):
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                if order_by == "ASC" and id_genre_sel == 0:
                    strsql_piece_afficher = """SELECT Id_piece, Nom_piece, Surface_piece FROM t_piece ORDER BY Id_piece ASC"""
                    mc_afficher.execute(strsql_piece_afficher)
                elif order_by == "ASC":
                    ...
                    valeur_id_piece_selected_dictionnaire = {"value_id_genre_selected": id_genre_sel}
                    strsql_piece_afficher = """SELECT Id_piece, Nom_piece, Surface_piece FROM t_piece WHERE Id_piece = %(value_id_genre_selected)s"""

                    mc_afficher.execute(strsql_piece_afficher, valeur_id_piece_selected_dictionnaire)
                else:
                    strsql_piece_afficher = """SELECT Id_piece, Nom_piece, Surface_piece FROM t_piece ORDER BY Id_piece DESC"""
    ...
    return render_template("piece/piece_afficher.html", data=data_genres)
    ...
    if request.method == "POST":
        try:
            if form.validate_on_submit():
                ...
                valeurs_insertion_dictionnaire = {"value_nom_piece": nom_personne,
                                                  "value_Surface_piece": prenom_personne}

                print("valeurs_insertion_dictionnaire ", valeurs_insertion_dictionnaire)

                strsql_insert_genre = """INSERT INTO t_piece (Id_piece,Nom_piece,Surface_piece) VALUES (NULL,%(value_nom_piece)s,%(value_Surface_piece)s) """
...
def pieces_update_wtf():
    ...
    try:
        ...
        if form_update.validate_on_submit():
            ...
            return redirect(url_for('genres_afficher', order_by="ASC", id_pieces_sel=id_genre_update))
        elif request.method == "GET":
            # Opération sur la BD pour récupérer "Id_personne" et "Nom_personne" de la "t_personne"
            str_sql_id_genre = "SELECT Id_piece, Nom_piece, Surface_piece FROM t_piece WHERE Id_piece = %(value_id_piece)s"
    ...
    try:
        ...
        if request.method == "POST" and form_delete.validate_on_submit():
            ...
            if form_delete.submit_btn_annuler.data:
                return redirect(url_for("genres_afficher", order_by="ASC", id_pieces_sel=0))
